...
# replace 'Example Domain' with what you want
stringForChecking = "Example Domain"
...
with open("URLs.txt", encoding="utf8") as file:
    for line in file:
        ...
        print(time.strftime("%H:%M:%S", time.localtime()),
              line, "end page loading", driver.current_url)
        # pause after adding of one url to avoid your IP blocking
        time.sleep(60)
        assert stringForChecking in driver.title
        print(time.strftime("%H:%M:%S", time.localtime()),
              "page added", line, driver.current_url)
driver.quit()
